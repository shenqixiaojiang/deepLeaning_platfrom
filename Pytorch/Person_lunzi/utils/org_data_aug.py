...
class RandomResizedCrop(object):
    ...
    def __call__(self,img):
        if random.random() < 0.2:
            return cv2.resize(img,self.size,interpolation=cv2.INTER_CUBIC)
        h, w, _ = img.shape
        area = h * w
        d = 1
...
class RandomHShift(object): ## change on the left and right part
...
class ExpandBorder(object):  ##It make the image having the same width and height... if resize, then they have (resize_width,resize_height)
    ...
    def __call__(self, image):
        ...
        if self.resize:
            image = cv2.resize(image, (self.size[0], self.size[0]),interpolation=cv2.INTER_CUBIC)  ##cv2.INTER_CUBIC,cv2.INTER_AREA
